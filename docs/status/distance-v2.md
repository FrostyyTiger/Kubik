...
### And the pictures, which character v2 gave a reason to doubt

Character v2 merged the same day and its Stage 0 found that **the character
gallery's sheets are not bit-reproducible on this GPU** - a fresh shot against
another fresh shot of the same commit differed on 31 of 53 sheets, mostly one
least-significant bit over lit surfaces plus antialiased edges flipping
coverage. Several numbers in this document are per-pixel comparisons of tour
shots, so that finding is either a problem for them or it is not, and it is
worth saying which.

**It is not, and there is a control that proves it.** The same Stage 0 records
that anything drawn with the **terrain** material comes back IDENTICAL -
`swatches.png`, `swatch-ramp.png` - and the tour shots this epic compares are
terrain and flora. The control is Stage 7's: two tours taken hours and four
commits apart, both at `far_terrace 0.0`, compared over the whole far-massif
band, came back at **mean |dL| 0.0000, worst 0.0** - not "close", identical.

So the null measurement in this document really is null, and the small numbers
next to it - 0.23 sRGB levels for the band lock, 1,063 pixels for the
