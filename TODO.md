...
*2026-09-04: the reconciliation's order (`RECONCILIATION.md` § 9, `CLAUDE.md` § Working order) is the queue now, reordered by Marcel for the north star (D84). Everything below A9 is the queue behind it, and the lettered waves are what fills phases 3 to 6. 2026-09-05: A10 goes in front of A9 (D85) - the frame before world truth.*

- [x] **A7. Horizon v1 - the view to the horizon, and a world with no edge** (3 nights, `docs/plans/horizon-v1.md`) - **DONE**, merged 2026-09-05 from `feat/horizon-v1`, self-merged per Marcel's amendment. `docs/status/horizon-v1.md`. **Median sprint frame 41.67 -> 16.67 ms at Ultra with the view 3.2 -> 32 km**, canonical world line unchanged. The hitch half of the frame gate (171-233 frames of 3,340 over 25 ms) is open and is A10's; `far_ring_div` 4 -> 2 is an open ruling for Marcel. The north star: the world as big as the view, the view to 32 km, 60 FPS at max settings on mid hardware. Origin-anchored height tiles at every level, voxels anywhere, persistent per-ring/per-sector far meshes, one material source, a fog ramp on the draw distance, a floating origin, the sprint probe. Changes nothing a seed produces. `||` with A8.
...
- [ ] **A10. Upload v1 - the frame thread stops touching the mesh** (one night and a day, `docs/plans/upload-v1.md`; phase 1d, D85) - **LAUNCHED 2026-09-05** on `feat/upload-v1`, ganymede tmux `upload-v1`, alone. With generation and meshing off the main thread, what is left on it is `add_surface_from_arrays` plus a collision shape per column, 214 columns a second, and horizon v1's Stage 7 measured that as the whole hitch column - and that a smaller upload slice makes both numbers worse. Fewer, larger surfaces per column, or a mesh handed to the rendering server without the frame thread touching it. Look-only; changes nothing a seed produces. Gate: the sprint line at Ultra, 32 km, no frame over 25 ms, with the C++ chunk mesher on.
- [ ] **A9. The world-truth break** (weeks, draft `docs/plans/world-truth-v1.md`; D56 as amended by D84 and D85) - real relief (D45), rings from the capital (D44), lakes and zones per tile, the generator's truth in C++. Starts the day A10 lands. Then people and fire, buildings, the round 3 scene (`RECONCILIATION.md` § 9 phases 3 to 5).
...
- [ ] ~~**A6. Look v3 - the painted world**~~ - **OUT** (2026-09-03, `RECONCILIATION.md` § 8: replaced by the round 3 test scene; kept here so the number is not reused). Was a register change - "Art Deco fantasy", one painted-voxel surface language at every distance, with the KNIGHT TEST and the BELONGING TEST as its gates - recorded in a `docs/DESIGN.md` § Art direction that no longer exists. The bible replaces the argument outright: art pillar 2 is real light on flat cubes through a film lens with no textures on anything ever, art pillar 5 puts deco on the built and never on nature, and Art Nouveau is on paper only (D2). Light v1 built pillar 2 (`docs/status/light-v1.md`); what look v3 was going to prove is proved by the round 3 scene instead. Trees were already discharged by trees v3; the one shape that epic lost outright, the krummholz, needs new art and is `../Kubik-assets` work.
